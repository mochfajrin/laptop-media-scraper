...
export default async function getAllCpu(url: string) {
  ...
  try {
    ...
    const cpus = $("tbody>.cpu-row.w-full")
      .map(function () {
        ...
        const chinabench: number = Number($(this).find("td:eq(2)").text());
    ...
    await fs.writeFile(
      "./data/laptop-media-cpu-list.json",
...
getAllCpu("https://laptopmedia.com/top-laptop-cpu-ranking/");
